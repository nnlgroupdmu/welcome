- [[#摘要|摘要]]
	- [[#摘要#一、 整体架构概览|一、 整体架构概览]]
	- [[#摘要#二、 日常工作流程|二、 日常工作流程]]
	- [[#摘要#三、 核心注意事项|三、 核心注意事项]]
- [[#1  准备工作|1  准备工作]]
	- [[#1  准备工作#1.1 实验室外的连接准备|1.1 实验室外的连接准备]]
	- [[#1  准备工作#1.2 注册个人账号|1.2 注册个人账号]]
	- [[#1  准备工作#1.3 首次 ssh 连接|1.3 首次 ssh 连接]]
	- [[#1  准备工作#1.4 免密登录|1.4 免密登录]]
- [[#2  基于 Docker 的工作流|2  基于 Docker 的工作流]]
	- [[#2  基于 Docker 的工作流#2.1 **使用镜像创建容器**|2.1 **使用镜像创建容器**]]
	- [[#2  基于 Docker 的工作流#2.2 **环境配置**|2.2 **环境配置**]]
		- [[#2.2 **环境配置**#**怎么写清单？**|**怎么写清单？**]]
		- [[#2.2 **环境配置**#**如何一句话安装？**|**如何一句话安装？**]]
- [[#3  Tmux 终端复用|3  Tmux 终端复用]]
	- [[#3  Tmux 终端复用#3.1. 推荐的工作流程|3.1. 推荐的工作流程]]
		- [[#3.1. 推荐的工作流程#第一步：创建并进入会话|第一步：创建并进入会话]]
		- [[#3.1. 推荐的工作流程#第二步：日常操作（记住快捷键前缀 `Ctrl+B`）|第二步：日常操作（记住快捷键前缀 `Ctrl+B`）]]
		- [[#3.1. 推荐的工作流程#第三步：安全脱离（Detach）|第三步：安全脱离（Detach）]]
		- [[#3.1. 推荐的工作流程#第四步：重新连接（Attach）|第四步：重新连接（Attach）]]
	- [[#3  Tmux 终端复用#3.2 与 Docker 的完美配合|3.2 与 Docker 的完美配合]]
		- [[#3.2 与 Docker 的完美配合#**在命令行中指定显卡**|**在命令行中指定显卡**]]
	- [[#3  Tmux 终端复用#3.3 tmux 常用命令|3.3 tmux 常用命令]]
