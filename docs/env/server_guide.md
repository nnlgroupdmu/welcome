...
欢迎使用实验室 **5090 Blackwell 架构服务器**。本服务器专为高性能深度学习任务设计，面向多数同学的日常科研需求，为了确保资源的高效利用与环境的稳定隔离，请遵循我们的工作流程。
...
Beta: 如果您需要寻求 AI 帮助，可以在对话的开头使用如下的提示词：
```Prompt
...
> [!info]
> 「yf5090」是一个管理员设置的 Tailscale 魔法 DNS 链接，其等价于「192.168.31.240」。只要你连接了 Tailscale，就可以在 VS Code、浏览器等处使用「yf5090」代替数字地址。然而，其依赖 DNS 解析，与 Clash/本机设置 发生冲突等不能正常使用的情况，请使用完整链接「100.68.153.123」。
