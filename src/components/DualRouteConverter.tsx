...
export default function DualRouteConverter({ isCompact = false }: DualRouteConverterProps) {
  ...
  return (
    <div 
      ...
      className={isCompact 
        ? "bg-slate-50/70 hover:bg-slate-100/40 rounded-xl p-3 border border-slate-100/80 transition-all flex flex-col gap-2.5 text-left" 
        : "relative border border-dashed border-indigo-200 bg-linear-to-b from-white to-slate-50/40 rounded-xl p-4 transition-all hover:shadow-xs flex flex-col gap-3.5 text-left"
      }
    >
      {/* Target/Header line */}
      {!isCompact ? (
        ...
        <div className="flex items-center justify-between min-w-0">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="text-[11px] font-bold text-slate-700 truncate">双路地址智能转换</span>
          </div>
        </div>
      )}
      ...
      <div className="relative">
        <input 
          ...
          className={`w-full pr-8 border border-slate-200 rounded-lg text-xs font-mono bg-white focus:outline-none focus:ring-1 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all text-slate-700 placeholder:text-slate-400 shadow-xs ${
        ...
        {inputUrl && (
          <button
            onClick={() => setInputUrl('')}
            className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 font-bold text-sm bg-transparent border-0 cursor-pointer p-0.5 animate-fade-in"
            title="清除内容"
          >
            &times;
          </button>
      ...
      {/* Output Buttons Row */}
      <div className={`grid grid-cols-2 gap-2 w-full ${isCompact ? 'pt-0.5' : 'pt-1.5 border-t border-slate-100'}`}>
        {/* Tailscale Route Action */}
        <button
          id="btn-copy-card-ts"
          disabled={!resolvedTs}
          className={`px-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 border-solid text-xs font-bold rounded-lg flex items-center justify-center gap-1 transition duration-100 cursor-pointer ${
            isCompact ? 'py-1 text-[11px]' : 'py-1.5'
          } ${
            !resolvedTs 
              ? 'opacity-40 cursor-not-allowed bg-slate-50 text-slate-400 border-slate-200 shadow-none' 
              : 'cursor-pointer'
          }`}
        ...
        >
          {copiedType === 'ts' ? (
            <>
              <Check className="w-3 h-3 text-emerald-600 animate-bounce shrink-0" />
              <span>已复制 IP</span>
            </>
          ) : (
            <>
              <Copy className="w-3 h-3 text-emerald-600 shrink-0" />
        ...
        <button
          ...
          className={`px-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-800 border border-indigo-200 border-solid text-xs font-bold rounded-lg flex items-center justify-center gap-1 transition duration-100 cursor-pointer ${
            isCompact ? 'py-1 text-[11px]' : 'py-1.5'
          } ${
            !resolvedLan 
              ? 'opacity-40 cursor-not-allowed bg-slate-50 text-slate-400 border-slate-200 shadow-none' 
        ...
        >
          {copiedType === 'lan' ? (
            <>
              <Check className="w-3 h-3 text-indigo-600 animate-bounce shrink-0" />
              <span>已复制 IP</span>
            </>
          ) : (
            <>
