...
> [!info]
> 「yf5090」是一个管理员设置的 Tailscale 魔法 DNS 链接，其等价于「192.168.31.240」。只要你连接了 Tailscale，就可以在 VS Code、浏览器等处使用「yf5090」代替数字地址。然而，其依赖 DNS 解析，与 Clash/本机设置 发生冲突等不能正常使用的情况，请关闭 TUN 模式或使用完整链接「100.68.153.123」。
